...
def _count_cancellations(timeout, times=1):
    cancel_count = 0
...
@pytest.mark.boxed
def test_async_loop_supervisor_supervise(supervisor):
    supervisor.timeout = 1
    cancellations = supervisor.supervise(
        _count_cancellations(0.1, times=1))
...
@pytest.mark.boxed
def test_async_loop_supervisor_interrupt(supervisor):
    ...
    t = threading.Timer(0.1, _interrupt, (0.1, 1))
    t.start()
    supervisor.timeout = 0.1

    cancellations = supervisor.supervise(
        _count_cancellations(0.2, times=2))
